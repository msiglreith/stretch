...
use crate::node::{Storage, Stretch};
...
impl Stretch {
    pub(crate) fn compute(&mut self, root: NodeId, size: Size<Number>) -> Result<()> {
        let has_root_min_max = self.style[&root].min_size.width.is_defined()
            || self.style[&root].min_size.height.is_defined()
            || self.style[&root].max_size.width.is_defined()
            || self.style[&root].max_size.height.is_defined();

        let result = if has_root_min_max {
            let first_pass = self.compute_internal(
                root,
                Size {
                    width: self.style[&root].size.width.resolve(size.width),
                    height: self.style[&root].size.height.resolve(size.height),
                },
                size,
                false,
            )?;
            ...
            self.compute_internal(
                ...
                Size {
                    width: first_pass
                        .size
                        .width
                        .maybe_max(self.style[&root].min_size.width.resolve(size.width))
                        .maybe_min(self.style[&root].max_size.width.resolve(size.width))
                        .to_number(),
                    height: first_pass
                        .size
                        .height
                        .maybe_max(self.style[&root].min_size.height.resolve(size.height))
                        .maybe_min(self.style[&root].max_size.height.resolve(size.height))
                        .to_number(),
                },
                size,
                true,
    ...
    fn compute_internal(
        ...
        *self.is_dirty.get_mut(&node).unwrap() = false;
        ...
        let mut flex_items: Vec<FlexItem> = self.children[&node]
            .iter()
            .filter(|child| self.style[child].position_type != PositionType::Absolute)
            .filter(|child| self.style[child].display != Display::None)
            .map(|child| FlexItem {
                node: *child,
        ...
        flex_items.iter_mut().try_for_each(|child| -> Result<()> {
            ...
            if let Defined(ratio) = self.style[&child.node].aspect_ratio {
                if let Defined(cross) = node_size.cross(dir) {
                    if self.style[&child.node].flex_basis == Dimension::Auto {
            ...
            let width: Number = if !child.size.width.is_defined()
                && self.style[&child.node].align_self(&self.style[&node]) == AlignSelf::Stretch
                && is_column
            {
        ...
        fn calc_baseline(ui: &Stretch, node: NodeId, layout: &result::Layout) -> f32 {
            if ui.children[&node].is_empty() {
                layout.size.height
            } else {
                let child = ui.children[&node][0];
                calc_baseline(ui, child, &ui.layout[&child])
            }
        };
        ...
        flex_lines.iter_mut().for_each(|line| {
            ...
            line.items.iter_mut().for_each(|child| {
                child.target_size.set_cross(
                    dir,
                    if self.style[&child.node].align_self(&self.style[&node]) == AlignSelf::Stretch
                        && self.style[&child.node].cross_margin_start(dir) != Dimension::Auto
                        && self.style[&child.node].cross_margin_end(dir) != Dimension::Auto
                        && self.style[&child.node].cross_size(dir) == Dimension::Auto
                    {
        ...
        flex_lines.iter_mut().for_each(|line| {
            ...
            line.items.iter_mut().for_each(|child| {
                if self.style[&child.node].main_margin_start(dir) == Dimension::Auto {
                    num_auto_margins += 1;
                }
                if self.style[&child.node].main_margin_end(dir) == Dimension::Auto {
                    num_auto_margins += 1;
                }
            });
        ...
        flex_lines.iter_mut().for_each(|line| {
            ...
            line.items.iter_mut().for_each(|child| {
                ...
                if self.style[&child.node].cross_margin_start(dir) == Dimension::Auto
                    && self.style[&child.node].cross_margin_end(dir) == Dimension::Auto
                {
                    if is_row {
                ...
                } else if self.style[&child.node].cross_margin_start(dir) == Dimension::Auto {
                    if is_row {
                        child.margin.top = free_space;
                    } else {
                        child.margin.start = free_space;
                    }
                } else if self.style[&child.node].cross_margin_end(dir) == Dimension::Auto {
                    if is_row {
        ...
        // Before returning we perform absolute layout on all absolutely positioned children
        {
            ...
            for (order, child) in candidates {
                ...
                let start = self.style[&child].position.start.resolve(container_width)
                    + self.style[&child].margin.start.resolve(container_width);
                let end = self.style[&child].position.end.resolve(container_width)
                    + self.style[&child].margin.end.resolve(container_width);
                let top = self.style[&child].position.top.resolve(container_height)
                    + self.style[&child].margin.top.resolve(container_height);
                let bottom = self.style[&child].position.bottom.resolve(container_height)
                    + self.style[&child].margin.bottom.resolve(container_height);

                let (start_main, end_main) = if is_row { (start, end) } else { (top, bottom) };
                let (start_cross, end_cross) = if is_row { (top, bottom) } else { (start, end) };

                let width = self.style[&child]
                    .size
                    .width
                    .resolve(container_width)
                    .maybe_max(self.style[&child].min_size.width.resolve(container_width))
                    .maybe_min(self.style[&child].max_size.width.resolve(container_width))
                ...
                let height = self.style[&child]
                    .size
                    .height
                    .resolve(container_height)
                    .maybe_max(self.style[&child].min_size.height.resolve(container_height))
                    .maybe_min(self.style[&child].max_size.height.resolve(container_height))
                ...
                let offset_cross = if start_cross.is_defined() {
                    ...
                    match self.style[&child].align_self(&self.style[&node]) {
                        AlignSelf::Auto => 0.0, // Should never happen
                        AlignSelf::FlexStart => {
                            if is_wrap_reverse {
